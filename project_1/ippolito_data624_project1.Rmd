...
# Revert dataframe back to the earlier state, retaining weekend gaps
df2 <- df_orig %>%
    filter(SeriesInd <= 43021) %>%
    arrange(SeriesInd, category)
...
colnames(dfr) <- c('Category', 'Variable', 'Model', 'Method', 'MAPE')
...
dfr2 <- dfr %>%
    ...
    slice_min(MAPE)
colnames(dfr2) <- c('Category', 'Variable', 'Model', 'Method', 'MAPE')
dfr2 %>%
