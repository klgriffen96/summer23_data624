...
## Below here is just some code to make NAs provided in original file into Inf 
## AND make the skipped time seriesInds into NAs just so there is a distinction

# Convert NAs to Inf
for (i in 1:length(df_all_given)){
  df_all_given[[i]][is.na( df_all_given[[i]])] <- Inf
}

# Make the skipped time values NAs
df_all_series <- df_all_given

all_series <- c(min(df_all_given$S01$SeriesInd):max(df_all_given$S01$SeriesInd))
