...
Now that we have a full data set will missing values handled, we can generate a second, reduced set to use with models that are sensitive to irregularities. We'll first make a copy of the full set. Then we'll need to remove one of the Brand.Code columns, as having all categories of a categorical variable will be problemmatic for linear-based models.
...
# Remove the first dummy Brand.Code from the reduced set, since this will be problemmatic for linear models which use a y-intercept
dfm_red <- dfm_red %>%
    select(-Brand.Code_A)
...
We also gauged accuracy against how long it took to train the various models (see plot below). As expected, the more accurate models generally took much longer to run than those that were less accurate. With a run time approaching 4 minutes, the model with the best MAPE score was also one of the longest to train, despite using 8 Pentium i5 processing cores on the modeling machine.
