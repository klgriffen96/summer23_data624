...
countOutliers <- function(x, mult) {
    vals <- x[!is.na(x)]
    mean_val <- mean(vals)
    loval <- mean_val - (sd(vals) * mult)
    hival <- mean_val + (sd(vals) * mult)
    num_outliers <- length(vals[vals < loval | vals > hival])
}
...
To better understand the outliers
