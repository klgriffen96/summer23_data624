...
As previously noted, there is a single observation that has twelve missing predictor values. We'll remove this observation as it likely would contribute little information to the model.
...
*[if time, generate plots showing 2 of the most influential variables, with a color parameter of the variable to be imputed, coloring the missing values in red]
...
It is noted that after the dummy columns are created, the data set now has 35 predictors instead of 32.
...
First, we'll make a copy of the full data set (prior to splitting into training and test sets).

```{r}

# Copy the full set to the reduced set
dfm_red <- dfm_full
...
skewed_vars <- c('MFR', 'Filler.Speed', 'Oxygen.Filler', 'Temperature', 'Air.Pressurer', 'PSC.CO2')
...
Based on the outlier plots from earlier, most of the data points beyond the three-standard-deviation cutoff appear to be valid data points, though located on the extreme outer edges of the main body of data. One exception is Alch.Red, which includes six somewhat suspect points located along the left- and right-hand margins of the plot. The other possible exception is data point exhibiting a PH value of 9.36. It is unclear whether these are truly outliers that should be excluded. But in either case, they are points of high influence and they are small in number, so we chose to remove them.
...
After all modifications to the reduced data set have been made, our final version of the reduced set contains 2563 observations of 29 predictors, reduced from the full set of 2570 observations of 35 predictors. As with the full data set, we'll split the reduced set into training and test sets and verify that the number of predictors doesn't exceed the number of observations.
...
The table above shows that the test set contains 511 observations, far exceeding the number of predictors (29), so we won't need to worry about models that perform poorly when there are more predictors than observations.

## Modeling

Now that we have a full and reduced set of data--and training and test sets within those two sets--we can begin modeling. We'll start by performing linear regression-based modeling on the reduced data set.

```{r}

# Create a data frame to store the results
dfr <- data.frame(matrix(nrow=0, ncol=5))
colnames(dfr) <- c('Data.Set', 'Model', 'Tuning.Parameters', 'RMSE.Train', 'RMSE.Test')
...
#plot(fitbag_full)  # no tuning parameters for this kind of model, so no plot
...
dfr %>%
    arrange(RMSE.Test) %>%
    select(Model, Data.Set, Tuning.Parameters, RMSE.Train, RMSE.Test) %>%
    flextable() %>%
    width(width = 2) %>%
    fontsize(size = 10) %>%
    line_spacing(space = 1) %>%
...
dfr %>%
    ggplot(aes(x=reorder(Model, desc(RMSE.Test)), y=RMSE.Test, fill=Data.Set)) +
