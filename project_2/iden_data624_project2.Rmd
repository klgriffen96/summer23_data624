...
```{r, include=FALSE}
setwd('/Users/joshiden/Documents/Classes/CUNY SPS/Summer 2023/DATA624/PROJECTS/PJ2')
```

```{r, warning=FALSE, message=FALSE, cache=TRUE, error=FALSE}
# local filepaths
...
```{r}
kable(cbind("modeling data"=colnames(model.data),"evaluation data"=colnames(evaluation.data)), caption="column names") |> kable_styling()
```
...
8.25% of the `mfr` variable is missing, and 4.7% of the `brand.code`s are NA -- we will impute these using K-nearest neighbors.
...
Next we center and scale the variables and perform Box-Cox transformations on the skewed variables using the `preProcess` function,

```{r}
set.seed(1)
transformed <- preProcess(md.imputed,
                          method = c("BoxCox","center","scale"))

# apply the transformed data 
df <- predict(transformed, md.imputed)
df$ph <- md.imputed$ph
```
...
# set split index 
split.index <- createDataPartition(df$ph, p = .8, list = FALSE)

# split data and partition predictor (x) and response (y) sets
train.x <- df[split.index, ] |> select(-ph)
train.y <- df[split.index, ]$ph

test.x <- df[-split.index, ] |> select(-ph)
test.y <- df[-split.index, ]$ph
```
...
lm.results <- postResample(lm.preds, test.y)
stopCluster(cl)
registerDoSEQ()
```
...
pls.results <- postResample(pls.preds, test.y)
...
ridge.results <- postResample(ridge.preds, test.y)
...
enet.results <- postResample(enet.preds, test.y)
...
knn.results <- postResample(knn.preds, test.y)
...
nnet.results <- postResample(nnet.preds, test.y)
...
mars.results <- postResample(mars.preds, test.y)
...
svmR.results <- postResample(svmR.preds, test.y)
...
rpart.results <- postResample(rpart.preds, test.y)
```
...
bagged.results <- postResample(bagged.preds, test.y)
```
...
rf.results <- postResample(rf.preds, test.y)
```
...
gbm.model <- train(train.x, train.y,
                  ...
                  tuneGrid = gbm.grid,
                  verbose = FALSE,
                  trControl = parallel.config)

gbm.preds <- predict(gbm.model, test.x)
gbm.results <- postResample(gbm.preds, test.y)
...
cubist.results <- postResample(cubist.preds, test.y)
