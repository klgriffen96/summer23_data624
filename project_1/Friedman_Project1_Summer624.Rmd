...
```{r data-exploration}
lapply(list_vars, function(var) {
  ts <- myts[[var]]
  plot <- autoplot(ts) + ggtitle(paste("Plot for", var))
  print(plot)
})
```
...
## Run 
for (i in seq_along(myts)) {
  arima_MAPE<- arima_test(myts[[i]], i)
}
```
...
Because the SES model produces the best result, we will use that for all data tables

```{r forecasts}
forecasts <- lapply(myts, function(x){

  ses.fit <- ses(x, h = 140)$fitted
})
```
```{r export}
