...
1) Remove the row with a large number of missing values (14 in this case).
1) Remove the MFR predictor because it has a high proportion of missing values (31 values, or 11.6% of the data).
1) Change the blank brand codes to NA so that they will be imputed.
1) Impute the missing values using KNN imputation.
1) Create dummy variables for the Brand.Code predictor.
